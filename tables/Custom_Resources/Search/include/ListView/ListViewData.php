...
class ListViewData {
	...
	//====================================== JL added =========================
	function getInnerJoinSQL()
	{	
		//Return an empty INNER JOIN query if user has not specified a participant's first name.
		if(empty($_REQUEST['participant_first_name_basic']) && empty($_REQUEST['participant_last_name_basic']))
		{
			return "";
		}	
		...
		$rel_query = "SELECT *
						FROM relationships
						WHERE relationship_name = 'plt_lkprsprtcpt_plt_participant'
						OR relationship_name = 'plt_lkprsprtcpt_plt_person'";
						
		$result = 	$GLOBALS['db']->query($rel_query);	
		
		
		if($GLOBALS['db']->getAffectedRowCount() == 2)
		{			
			//set up join keys
			$plt_lkprsprtcpt_plt_participant_left_key = "";
			$plt_lkprsprtcpt_plt_participant_right_key = "";
			$plt_lkprsprtcpt_plt_person_left_key = "";
			$plt_lkprsprtcpt_plt_person_right_key = "";			
			
			while($row = $GLOBALS['db']->fetchByAssoc($result)) {				
							
				//Build dynamic sql query here.						
				if($row['relationship_name'] == "plt_lkprsprtcpt_plt_participant")
				{
					$plt_lkprsprtcpt_plt_participant_left_key = $row['join_key_lhs'];
					$plt_lkprsprtcpt_plt_participant_right_key = $row['join_key_rhs'];
				}
				elseif($row['relationship_name'] == "plt_lkprsprtcpt_plt_person")
				{
					$plt_lkprsprtcpt_plt_person_left_key = $row['join_key_lhs'];
					$plt_lkprsprtcpt_plt_person_right_key = $row['join_key_rhs'];
				}
				else{}
												
			}//end while		

			//Build INNER JOIN query since all variables needed are found.
			if(!empty($plt_lkprsprtcpt_plt_participant_left_key) && !empty($plt_lkprsprtcpt_plt_participant_right_key)	&& !empty($plt_lkprsprtcpt_plt_person_left_key)	&& !empty($plt_lkprsprtcpt_plt_person_right_key))
			{
				$inner_join_query = " INNER JOIN plt_lkprspr_participant_c ON plt_participant.id = plt_lkprspr_participant_c.".$plt_lkprsprtcpt_plt_participant_left_key." 
								INNER JOIN plt_lkprsprtcpt ON plt_lkprspr_participant_c.".$plt_lkprsprtcpt_plt_participant_right_key." = plt_lkprsprtcpt.id 
								INNER JOIN plt_lkprsprt_plt_person_c ON plt_lkprsprtcpt.id = plt_lkprsprt_plt_person_c.".$plt_lkprsprtcpt_plt_person_right_key."
								INNER JOIN plt_person ON (plt_lkprsprt_plt_person_c.".$plt_lkprsprtcpt_plt_person_left_key." = plt_person.id ";

				if(!empty($_REQUEST['participant_first_name_basic']))
				{
					$inner_join_query .= " AND plt_person.first_name LIKE '".$_REQUEST['participant_first_name_basic']."%'";
				}
				if(!empty($_REQUEST['participant_last_name_basic']))
				{
					$inner_join_query .= " AND plt_person.last_name LIKE '".$_REQUEST['participant_last_name_basic']."%'";
				}
				$inner_join_query .= " AND plt_lkprsprtcpt.relation='1' ) ";																
			}				
		}//end if
	...
	function getListViewData($seed, $where, $offset=-1, $limit = -1, $filter_fields=array(),$params=array(),$id_field = 'id') {
		...
		//==================================== added by JL ===================					
		//Array ( [module] => PLT_Participant [action] => index [searchFormTab] => basic_search [query] => true [name_basic] => [participant_first_name_basic] => FASDFA [enroll_date_basic] => [button] => Search ) 							
		if(!empty($_REQUEST['module']) && $_REQUEST['module'] == "PLT_Participant" && (!empty($_REQUEST['participant_first_name_basic']) || !empty($_REQUEST['participant_last_name_basic']) ))
		{		
			$ret_array['inner_join'] = $this->getInnerJoinSQL();
		}
		//==================================== end code added by JL =============

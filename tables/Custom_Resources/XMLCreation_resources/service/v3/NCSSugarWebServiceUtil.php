...
class NCSSugarWebServiceUtil extends SugarWebServiceUtilv3 {
    ...
    function parseStaff(&$xmlWriter, &$chunk)
    {
        ...
        while($val = $db->fetchByAssoc($results, -1, false)) {
			$xmlWriter->startElement('staff');
				...
				//$this->addXMLElement($xmlWriter, 'staff_type', $val['staff_type']);
				...
				$this->addXMLElement($xmlWriter, 'staff_gender', str_replace("_", "-", $val['staff_gender']));
    ...
    function parseStaffValidation(&$xmlWriter, &$chunk)
    {
        ...
        while($val = $db->fetchByAssoc($results, -1, false)) {
			$xmlWriter->startElement('staff_validation');
				...
				$staff_val_date = preg_split('/[ ]/', $val['staff_val_date']);
				$this->addXMLElement($xmlWriter, 'staff_val_date', $staff_val_date[0]);
    ...
    function parseOutreachRace(&$xmlWriter, &$chunk) 
    {
        ...
        while($val = $db->fetchByAssoc($results, -1, false)) {
            foreach (explode("^,^",str_replace("_", "-", $val['outreach_race2'])) as $raceval)
            {
				$raceval = str_replace("^", "", $raceval);
	...
	function parseOutreachTarget(&$xmlWriter, &$chunk)
    {
        ...
        while($val = $db->fetchByAssoc($results, -1, false)) {
            foreach (explode("^,^", str_replace("_", "-", $val['outreach_target_ms'])) as $targetval)
            {
				$targetval = str_replace("^", "", $targetval);
	...
	function parseOutreachLanguage2(&$xmlWriter, &$chunk)
    {
        ...
        while($val = $db->fetchByAssoc($results, -1, false)) {
            foreach (explode("^,^", str_replace("_", "-", $val['outreach_lang2'])) as $langval)
            {
				$langval = str_replace("^", "", $langval);
				$xmlWriter->startElement('outreach_lang2');
					$this->addXMLElement($xmlWriter, 'psu_id', $this->master_psu_id);
					$this->addXMLElement($xmlWriter, 'outreach_lang2_id', $val['name'] . '-' . $langval);
					$this->addXMLElement($xmlWriter, 'outreach_event_id', $val['st_msoutlant_wkoeact_name']);
					$this->addXMLElement($xmlWriter, 'outreach_lang2', $langval);
					$this->addXMLElement($xmlWriter, 'transaction_type', 'NA');
				$xmlWriter->endElement();
            }
        }
    ...
    function parseStaffCertTraining(&$xmlWriter, &$chunk)
    {
        ...
        while($val = $db->fetchByAssoc($results, -1, false)) {
			$xmlWriter->startElement('staff_cert_training');
				...
				$cert_date = preg_split('/[ ]/', $val['cert_date']);
				$this->addXMLElement($xmlWriter, 'cert_date', $cert_date[0]);
    ...
    function parsePerson(&$xmlWriter, &$chunk)
    {
        ...
        while($val = $db->fetchByAssoc($results, -1, false)) {
            ...
            $xmlWriter->startElement('person');
				...
				$this->addXMLElement($xmlWriter, 'deceased', str_replace("_", "-", $val['deceased']));
				...
				$date_move = preg_split('/[ ]/', $val['date_move']);
				$this->addXMLElement($xmlWriter, 'date_move', $date_move[0]);
                // $this->addXMLElement($xmlWriter, 'p_tracing', $val['p_tracing']);
				$this->addXMLElement($xmlWriter, 'p_tracing', str_replace("_", "-", $val['p_tracing']));
    ...
    function parseParticipant(&$xmlWriter, &$chunk)
    {
        ...
        while($val = $db->fetchByAssoc($results, -1, false)) {
			$xmlWriter->startElement('participant');
				...
				$status_info_date = preg_split('/[ ]/', $val['status_info_date']);
				$this->addXMLElement($xmlWriter, 'status_info_date', $status_info_date[0]);
    ...
    function parseParticipantConsent(&$xmlWriter, &$chunk) 
    {
        ...
        while($val = $db->fetchByAssoc($results, -1, false)) {
            $xmlWriter->startElement('participant_consent');
				...
				$this->addXMLElement($xmlWriter, 'consent_withdraw', str_replace("_", "-", $val['consent_withdraw']));
    ...
    function parseProvider(&$xmlWriter, &$chunk)
    {
        ...
        while($val = $db->fetchByAssoc($results, -1, false)) {
			$xmlWriter->startElement('provider');
				...
				$this->addXMLElement($xmlWriter, 'provider_info_source', str_replace("_", "-", $val['provider_info_source']));
	...
	function parseProviderRole(&$xmlWriter, &$chunk)
    {
        ...
        while($val = $db->fetchByAssoc($results, -1, false)) {
            foreach (explode("^,^", str_replace("_", "-", $val['provider_ncs_role'])) as $roleval)
            {
				$roleval = str_replace("^", "", $roleval);
				$xmlWriter->startElement('provider_role');
					$this->addXMLElement($xmlWriter, 'psu_id', $this->master_psu_id);
					$this->addXMLElement($xmlWriter, 'provider_role_id', $val['name'] . '-' . $roleval);
    ...
    function parseInstitution(&$xmlWriter, &$chunk)
    {
        ...
        while($val = $db->fetchByAssoc($results, -1, false)) {
            $xmlWriter->startElement('institution');
				...
				$this->addXMLElement($xmlWriter, 'institute_info_source', str_replace("_", "-", $val['institute_info_source']));
    ...
    function parseAddress(&$xmlWriter, &$chunk)
    {
        ...
        while($val = $db->fetchByAssoc($results, -1, false)) {
            ...
            $xmlWriter->startElement('address');
				...
				$address_start_date = preg_split('/[ ]/', $val['address_start_date']);
				$this->addXMLElement($xmlWriter, 'address_start_date', $address_start_date[0]);
                // $this->addXMLElement($xmlWriter, 'address_end_date', $val['address_end_date']);
				$address_end_date = preg_split('/[ ]/', $val['address_end_date']);
				$this->addXMLElement($xmlWriter, 'address_end_date', $address_end_date[0]);
				...
				$this->addXMLElement($xmlWriter, 'zip4', str_replace("_", "-", $val['zip4']));
    ...
    function parseTelephone(&$xmlWriter, &$chunk)
    {
        ...
        while($val = $db->fetchByAssoc($results, -1, false)) {
            $xmlWriter->startElement('telephone');
				...
				$phone_info_date = preg_split('/[ ]/', $val['phone_info_date']);
				$this->addXMLElement($xmlWriter, 'phone_info_date', $phone_info_date[0]);
                // $this->addXMLElement($xmlWriter, 'phone_info_update', $val['phone_info_update']);
				$phone_info_update = preg_split('/[ ]/', $val['phone_info_update']);
				$this->addXMLElement($xmlWriter, 'phone_info_update', $phone_info_update[0]);
				...
				$phone_end_date = preg_split('/[ ]/', $val['phone_end_date']);
				$this->addXMLElement($xmlWriter, 'phone_end_date', $phone_end_date[0]);
                $this->addXMLElement($xmlWriter, 'transaction_type', 'NA');
            $xmlWriter->endElement();
        }
        $xmlWriter->flush();
    ...
    function parseEmail(&$xmlWriter, &$chunk)
    {
        ...
        while($val = $db->fetchByAssoc($results, -1, false)) {
            $xmlWriter->startElement('email');
				...
				$email_end_date = preg_split('/[ ]/', $val['email_end_date']);
				$this->addXMLElement($xmlWriter, 'email_end_date', $email_end_date[0]);
                $this->addXMLElement($xmlWriter, 'transaction_type', 'NA');
            $xmlWriter->endElement();
        }
        $xmlWriter->flush();
	...
	function parseContact(&$xmlWriter, &$chunk)
	{
		...
		while($val = $db->fetchByAssoc($results, -1, false)) {
			$xmlWriter->startElement('contact');
				...
				$this->addXMLElement($xmlWriter, 'contact_disp', $val['contact_disp_c']);
				// $this->addXMLElement($xmlWriter, 'contact_type', $val['contact_type']);
				$this->addXMLElement($xmlWriter, 'contact_type', str_replace("_", "-", $val['contact_type_c']));
				$this->addXMLElement($xmlWriter, 'contact_type_oth', $val['contact_type_other_c']);
				$contact_date = preg_split('/[ ]/', $val['start_date_time_c']);
				$this->addXMLElement($xmlWriter, 'contact_date', $contact_date[0]);
				$contact_start_time = preg_split('/[ ]/', $val['start_date_time_c']);
				$this->addXMLElement($xmlWriter, 'contact_start_time', $contact_start_time[1]);
				$contact_end_time = preg_split('/[ ]/', $val['end_date_time_c']);
				...
				$this->addXMLElement($xmlWriter, 'contact_lang', str_replace("_", "-", $val['contact_language_c']));
				$this->addXMLElement($xmlWriter, 'contact_lang_oth', $val['contact_lang_oth_c']);
				// $this->addXMLElement($xmlWriter, 'contact_interpret', $val['contact_interpret']);
				$this->addXMLElement($xmlWriter, 'contact_interpret', str_replace("_", "-", $val['contact_interpret_c']));
				$this->addXMLElement($xmlWriter, 'contact_interpret_oth', $val['contact_interpret_oth_c']);
				// $this->addXMLElement($xmlWriter, 'contact_location', $val['contact_location']);
				$this->addXMLElement($xmlWriter, 'contact_location', str_replace("_", "-", $val['contact_location_c']));
				$this->addXMLElement($xmlWriter, 'contact_location_oth', $val['contact_location_oth_c']);
				// $this->addXMLElement($xmlWriter, 'contact_private', $val['contact_private']);
				$this->addXMLElement($xmlWriter, 'contact_private', str_replace("_", "-", $val['contact_private_c']));
				$this->addXMLElement($xmlWriter, 'contact_private_detail', $val['contact_private_detail_c']);
				$this->addXMLElement($xmlWriter, 'contact_distance', $val['contact_distance_c']);
				// $this->addXMLElement($xmlWriter, 'who_contacted', $val['who_contacted']);
				$this->addXMLElement($xmlWriter, 'who_contacted', str_replace("_", "-", $val['who_contacted_c']));
				$this->addXMLElement($xmlWriter, 'who_contact_oth', $val['who_contact_oth_c']);
				$this->addXMLElement($xmlWriter, 'contact_comment', $val['description']);
				$this->addXMLElement($xmlWriter, 'transaction_type', 'NA');
			$xmlWriter->endElement();
		}
                $xmlWriter->flush();
	...
	function parseNIRVacant(&$xmlWriter, &$chunk)
	{
		...
		while($val = $db->fetchByAssoc($results, -1, false)) {
			foreach (explode("^,^", str_replace("_", "-", $val['nir_vacant'])) as $vacantval)
			{
				$vacantval = str_replace("^", "", $vacantval);
	...
	function parseNIRNoAccess(&$xmlWriter, &$chunk)
	{
		...
		while($val = $db->fetchByAssoc($results, -1, false)) {
			foreach(explode("^,^", str_replace("_", "-", $val['nir_noaccess'])) as $noaccessval)
			{
				$noaccessval = str_replace("^", "", $noaccessval);
	...
	function parseNIRRefusal(&$xmlWriter, &$chunk)
	{
		...
		while($val = $db->fetchByAssoc($results, -1, false)) {
			foreach (explode("^,^", str_replace("_", "-", $val['refusal_reason'])) as $refusalval)
			{
				$refusalval = str_replace("^", "", $refusalval);
				$xmlWriter->startElement('non_interview_rpt_refusal');
					$this->addXMLElement($xmlWriter, 'psu_id', $this->master_psu_id);
					$this->addXMLElement($xmlWriter, 'nir_refusal_id', $val['name'] . '-' . $refusalval);
	...
	function parseNIRDuType(&$xmlWriter, &$chunk)
	{
		...
		while($val = $db->fetchByAssoc($results, -1, false)) {
                    foreach (explode("^,^", str_replace("_", "-", $val['nir_type_du'])) as $dutypeval)
                    {
                        $dutypeval = str_replace("^", "", $dutypeval);
	...
	function parseIncident(&$xmlWriter, &$chunk)
	{
		...
		while($val = $db->fetchByAssoc($results, -1, false)) {
			$xmlWriter->startElement('incident');
				...
				$this->addXMLElement($xmlWriter, 'inc_staff_reporter_id', $val['inc_staff_reporter_id']);
				$this->addXMLElement($xmlWriter, 'inc_staff_supervisor_id', $val['inc_staff_supervisor_id']);
				$this->addXMLElement($xmlWriter, 'inc_recip_is_participant', $val['inc_recip_is_participant']);
				$this->addXMLElement($xmlWriter, 'inc_recip_is_du', $val['inc_recip_is_du']);
				$this->addXMLElement($xmlWriter, 'inc_recip_is_staff', $val['inc_recip_is_staff']);
				$this->addXMLElement($xmlWriter, 'inc_recip_is_family', $val['inc_recip_is_family']);
				$this->addXMLElement($xmlWriter, 'inc_recip_is_acquaintance', $val['inc_recip_is_acquaintance']);
				$this->addXMLElement($xmlWriter, 'inc_recip_is_other', $val['inc_recip_is_other']);
				$this->addXMLElement($xmlWriter, 'inc_contact_person', $val['inc_contact_person']);
				...
				$this->addXMLElement($xmlWriter, 'contact_id', $val['ncsdc_cntct_incident_name']);
				$this->addXMLElement($xmlWriter, 'transaction_type', 'NA');
			$xmlWriter->endElement();
		}
                $xmlWriter->flush();
	...
	function parseIncidentMedia(&$xmlWriter, &$chunk)
	{
		...
		while($val = $db->fetchByAssoc($results, -1, false)) {
                    foreach(explode("^,^", str_replace("_", "-", $val['incloss_media'])) as $mediaval)
                    {
                        $mediaval = str_replace("^", "", $mediaval);
	...
	function parseIncidentUnanticipated(&$xmlWriter, &$chunk)
	{
		...
		while($val = $db->fetchByAssoc($results, -1, false)) {
                    foreach(explode("^,^", str_replace("_", "-", $val['inc_unanticipated'])) as $unval)
                    {
                        $unval = str_replace("^", "", $unval);
			$xmlWriter->startElement('incident_unanticipated');
				$this->addXMLElement($xmlWriter, 'psu_id', $this->master_psu_id);
				$this->addXMLElement($xmlWriter, 'inc_unanticipated_id', $val['name'] . '-' . $unval);
				$this->addXMLElement($xmlWriter, 'incident_id', $val['ncsdc_incidcunatmlts_name']);
				$this->addXMLElement($xmlWriter, 'inc_unanticipated', $unval);
				$this->addXMLElement($xmlWriter, 'transaction_type', 'NA');
			$xmlWriter->endElement();
                    }
		}
    ...
    function parseSpecimenPickup(&$xmlWriter, &$chunk)
    {
		...
		//$pickup_date = preg_split('/[ ]/', $val['specimen_pickup_dt']);
		//$this->addXMLElement($xmlWriter, 'specimen_pickup_dt', $pickup_date[0].'T'.$pickup_date[1]);
                $specimen_pickup_dt = (strlen($val['specimen_pickup_dt']) > 0) ? date("Y-m-d\TH:i", strtotime($val['specimen_pickup_dt'])) : "";
                $this->addXMLElement($xmlWriter, 'specimen_pickup_dt', $specimen_pickup_dt);
    ...
    function parseSpecimenReceipt(&$xmlWriter, &$chunk)
    {
        ...
        while($val = $db->fetchByAssoc($results, -1, false)) {
            $xmlWriter->startElement('spec_receipt');
				...
				//$this->addXMLElement($xmlWriter, 'receipt_dt', $receipt_dt[0].'T'.$receipt_dt[1]);
                $receipt_dt = (strlen($val['receipt_dt']) > 0) ? date("Y-m-d\TH:i:s", strtotime($val['receipt_dt'])) : "";
                $this->addXMLElement($xmlWriter, 'receipt_dt', $receipt_dt);
    ...
    function parseSpecimenStorage(&$xmlWriter, &$chunk)
    {
        ...
        while($val = $db->fetchByAssoc($results, -1, false)) {
			$xmlWriter->startElement('spec_storage');
				...
				$temp_event_st = (strlen($val['temp_event_st']) > 0) ? date("Y-m-d\TH:i:s", strtotime($val['temp_event_st'])) : "";
                $this->addXMLElement($xmlWriter, 'temp_event_st', $temp_event_st);
                // $this->addXMLElement($xmlWriter, 'temp_event_et', $val['temp_event_et']);
				$temp_event_et = (strlen($val['temp_event_et']) > 0) ? date("Y-m-d\TH:i:s", strtotime($val['temp_event_et'])) : "";
				$this->addXMLElement($xmlWriter, 'temp_event_et', $temp_event_et);
    ...
    function parseEnvironmentalEquipmentProblemLog(&$xmlWriter, &$chunk)
    {
        ...
        while($val = $db->fetchByAssoc($results, -1, false)) {
			$xmlWriter->startElement('env_equipment_prob_log');
				$this->addXMLElement($xmlWriter, 'psu_id', $this->master_psu_id);
				$this->addXMLElement($xmlWriter, 'srsc_id', $val['samp_enloge_srscinfo_name']);
    ...
    function parseSampleReceiptStorage(&$xmlWriter, &$chunk)
    {
        ...
        while($val = $db->fetchByAssoc($results, -1, false)) {
			$xmlWriter->startElement('sample_receipt_store');
				...
				$this->addXMLElement($xmlWriter, 'equip_id', $val['samp_recstop_enequip_name']);
				// $this->addXMLElement($xmlWriter, 'placed_in_storage_dt', $val['placed_in_storage_dt']);
				//$placed_in_storage_dt = preg_split('/[ ]/', $val['placed_in_storage_dt']);
				//$this->addXMLElement($xmlWriter, 'placed_in_storage_dt', $placed_in_storage_dt[0].'T'.$placed_in_storage_dt[1]);
				$placed_in_storage_dt = (strlen($val['placed_in_storage_dt']) > 0) ? date("Y-m-d\TH:i", strtotime($val['placed_in_storage_dt'])) : "";
